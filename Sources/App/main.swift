...
drop.get("posts", Int.self) { request, postId in
  return "Requesting a post with ID \(postId)"
}

/**
  This will set up the appropriate GET, PUT, and POST
  routes for basic CRUD operations. Check out the
  UserController in <workDir>/Sources/App/Controllers to see more.
...
drop.get("session") { request in
  ...
  try request.session().data["name"] = Node.string("Vapor")
...
/**
  ...
  Check out the middleware in <workDir>/Sources/App/Middleware.
